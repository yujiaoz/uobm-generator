...
public class Generator {
	...
	private void _generateDept(int univIndex, int index, Specification spec) {
		String fileName = System.getProperty("user.dir")
				...
				+ _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + "dept"
				+ index + _getFileSuffix();
	...
	private void _generateAPerson_a(int type, int index) {
		writer_.addProperty(CS_P_SAMEHOMETOWN, _getADiffPerson(type, index),
				true);
	 ...
	 * 
	 * @param index
	 *            Index of void _generateAStudent_a(int type, int index) {
	 *            writer_.addProperty(CS_P_NAME, _getRelativeName(type, index),
	 *            false); writer_.addProperty(CS_P_ENROLLIN, _getId(CS_C_DEPT,
	 *            instances_[CS_C_DEPT].count - 1), true);
	 *            writer_.addProperty(CS_P_EMAIL, _getEmail(type, index),
	 *            false); writer_.addProperty(CS_P_TELEPHONE, "xxx-xxx-xxxx",
	 *            false); }the graduate course.
	 */
			...
			for (int i = 0; i < CLASS_NUM; ++i) {
				...
				case CS_C_UNDERSTUD:
					num[i] = _getRandomFromRange(R_UNDERSTUD_FACULTY_MIN
							* num[CS_C_FACULTY], R_UNDERSTUD_FACULTY_MAX
							* num[CS_C_FACULTY]);
					break;
				case CS_C_GRADSTUD:
					num[i] = _getRandomFromRange(R_GRADSTUD_FACULTY_MIN
							* num[CS_C_FACULTY], R_GRADSTUD_FACULTY_MAX
							* num[CS_C_FACULTY]);
					break;
				case CS_C_COURSE:
					num[i] = _getRandomFromRange(FACULTY_COURSE_MIN,
							FACULTY_COURSE_MAX);
					break;
				case CS_C_GRADCOURSE:
					num[i] = _getRandomFromRange(FACULTY_GRADCOURSE_MIN,
							FACULTY_GRADCOURSE_MAX);
					break;
				case CS_C_RESEARCHGROUP:
					num[i] = _getRandomFromRange(RESEARCHGROUP_MIN,
							RESEARCHGROUP_MAX);
					break;
				default:
	...
	public String getUnivInst() {
		return Class.getUnivID(Lib.getRandomFromRange(0, Class.UNIV_NUM));
	}
